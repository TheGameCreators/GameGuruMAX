...
function lifetap_spell_properties(e, prompt_text, useage_text, pickup_range, user_global_affected, mana_cost, cast_damage, cast_radius, player_level, particle1_name, particle2_name, prompt_display, item_highlight, highlight_icon_imagefile)
	...
	lifetap_spell[e].prompt_display = prompt_display	
	lifetap_spell[e].item_highlight = item_highlight
	lifetap_spell[e].highlight_icon = highlight_icon_imagefile
end

function lifetap_spell_init(e)
	lifetap_spell[e] = {}
	lifetap_spell[e].prompt_text = "E to Collect"
	lifetap_spell[e].useage_text = "You cast Lifetap and gained some health"
	...
	lifetap_spell[e].prompt_display = 1	
	lifetap_spell[e].item_highlight = 0	
	lifetap_spell[e].highlight_icon = "imagebank\\icons\\pickup.png"
...
function lifetap_spell_main(e)
	...
	if status[e] == "init" then
		if lifetap_spell[e].item_highlight == 3 and lifetap_spell[e].highlight_icon ~= "" then
			hl_icon[e] = CreateSprite(LoadImage(lifetap_spell[e].highlight_icon))
			hl_imgwidth[e] = GetImageWidth(LoadImage(lifetap_spell[e].highlight_icon))
			hl_imgheight[e] = GetImageHeight(LoadImage(lifetap_spell[e].highlight_icon))
		...
		if lifetap_spell[e].particle1_number == 0 and lifetap_spell[e].particle1_name ~= "" then
		...
		if lifetap_spell[e].particle2_number == 0 and lifetap_spell[e].particle2_name ~= "" then
	...
	if status[e] == "collect_spell" then
		...
		if PlayerDist < lifetap_spell[e].pickup_range then
			...
			module_misclib.pinpoint(e,lifetap_spell[e].pickup_range,lifetap_spell[e].item_highlight,hl_icon[e])
			sEnt[e] = g_tEnt
			--end pinpoint select object--	
		end	
		if PlayerDist < lifetap_spell[e].pickup_range and sEnt[e] == e then
			if GetEntityCollectable(e) == 1 then
				if GetEntityCollected(e) == 0 then
					if lifetap_spell[e].prompt_display == 1 then PromptLocal(e,lifetap_spell[e].prompt_text) end
					if lifetap_spell[e].prompt_display == 2 then Prompt(lifetap_spell[e].prompt_text) end
	...
	if status[e] == "have_spell" then	
		...
		if lifetap_spell[e].cast_timeout > 0 then
			if Timer() > lifetap_spell[e].cast_timeout + 2100 then
				...
				if lifetap_spell[e].particle1_number > 0 then Hide(lifetap_spell[e].particle1_number) end
				if lifetap_spell[e].particle2_number > 0 then Hide(lifetap_spell[e].particle2_number) end
				casttarget[e] = 0
			else
		...
		if tusedvalue > 0 and tTarget[e] ~= 0 then
			...
			if tplayerlevel[e] >= tlevelrequired[e] then
				...
				if mymana >= lifetap_spell[e].mana_cost then
					...
					if lifetap_spell[e].particle1_number > 0 then
					...
					if lifetap_spell[e].particle2_number > 0 then
