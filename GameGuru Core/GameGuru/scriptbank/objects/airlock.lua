...
-- Airlock Switch v6 by Necrym59 - a special thankyou to MonkeyFrog
...
-- DESCRIPTION: [PROMPT_TEXT$="Z=Depressurise  X=Pressurise"]
-- DESCRIPTION: [USE_RANGE=80(1,100)]
-- DESCRIPTION: [DEPRESSURISE_TEXT$="Depressurising Airlock"]
-- DESCRIPTION: [PRESSURISE_TEXT$="Pressurising Airlock"]
-- DESCRIPTION: [DOOR1_NAME$="Inner Door Name"]
-- DESCRIPTION: [DOOR2_NAME$="Outer Door Name"]
-- DESCRIPTION: [#DELAY=8(1,10000)]
-- DESCRIPTION: Play <Sound0> when activating
-- DESCRIPTION: Play <Sound1> when processing
...
function airlock_properties(e, airlock_type, air_level, water_level, prompt_text, use_range, depressurise_text, pressurise_text, door1_name, door2_name, door1_number, door2_number, delay, state)
	airlock[e] = g_Entity[e]
...
function airlock_main(e)
	...
	if status[e] == "init" then
		...
		if airlock[e].door1_number == 0 or nil then
		...
		if airlock[e].door2_number == 0 or nil then
	...
	if airlock[e].state == 1 then
		...
		if PlayerDist < airlock[e].use_range and g_PlayerHealth > 0 then
			...
			if LookingAt == 1 then
				PromptLocalForVR(e, airlock[e].prompt_text)
				if GetKeyState(44) == 1 then
					SetActivated(e,1)
					PlaySound(e,0)
	...
	if airlock[e].state == 2 then -- Close Both Doors
		...
		if mode[e] == "Depressurise" then
			TextCenterOnX(50,50,3,"Completion in: " ..countdown[e].. " seconds")
			TextCenterOnX(50,48,3,airlock[e].depressurise_text)
		end
		if mode[e] == "Pressurise" then
			TextCenterOnX(50,50,3,"Completion in: " ..countdown[e].. " seconds")
			TextCenterOnX(50,48,3,airlock[e].pressurise_text)
		end
	end

	if airlock[e].state == 3 then -- Open Outer Door
		LoopSound(e,1)
		if g_Entity[airlock[e].door2_number]['activated'] == 0 then
	...
	if airlock[e].state == 4 then -- Open Inner Door
		if g_Entity[airlock[e].door1_number]['activated'] == 0 then
