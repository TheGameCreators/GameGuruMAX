...
Sample_SoloMesh::Sample_SoloMesh() :
	m_keepInterResults(false), //PE: TEST, look fine :) outdoor jungle demo went from 2gb mem use to 29mb, what!
...
void Sample_SoloMesh::handleRender()
{
	...
	{
		// Draw mesh - great way to see if OBJ reduced geometry matches real static level
	...
	// when leaVe, set last slot so not to overwrite what we have above
...
bool Sample_SoloMesh::handleBuild()
{
	...
	//if(isExe == 0) printscreenprompt("rcMarkWalkableTriangles");
	rcMarkWalkableTriangles(m_ctx, m_cfg.walkableSlopeAngle, verts, nverts, tris, ntris, m_triareas);

	if(isExe == 0) printscreenprompt("RASTERIZING NAVIGATION MESH");
	...
	//if(isExe == 0) printscreenprompt("Partition walkable surface");
	...
	//(isExe == 0) printscreenprompt("Trace and simplify region contours");
	//
	...
	if(isExe == 0) printscreenprompt("BUILDING NAVIGATION MESH");
