...
static void mergeRegionHoles(rcContext* ctx, rcContourRegion& region)
{
	...
	if (!diags)
	{
		if ( ctx ) ctx->log(RC_LOG_WARNING, "mergeRegionHoles: Failed to allocated diags %d.", maxVerts);
		return;
	}
	...
	for (int i = 0; i < region.nholes; i++)
	{
		...
		if (index == -1)
		{
			if ( ctx ) ctx->log(RC_LOG_WARNING, "mergeHoles: Failed to find merge points for %p and %p.", region.outline, hole);
			continue;
		}
		if (!mergeContours(*region.outline, *hole, index, bestVertex))
		{
			if ( ctx ) ctx->log(RC_LOG_WARNING, "mergeHoles: Failed to merge contours %p and %p.", region.outline, hole);
			continue;
		}
	}
}
...
{	
	...
	if (!flags)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'flags' (%d).", chf.spanCount);
		return false;
	}
	
	if ( ctx ) ctx->startTimer(RC_TIMER_BUILD_CONTOURS_TRACE);
	...
	if ( ctx ) ctx->stopTimer(RC_TIMER_BUILD_CONTOURS_TRACE);
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				if ( ctx ) ctx->startTimer(RC_TIMER_BUILD_CONTOURS_SIMPLIFY);
				simplifyContour(verts, simplified, maxError, maxEdgeLen, buildFlags);
				removeDegenerateSegments(simplified);
				if ( ctx ) ctx->stopTimer(RC_TIMER_BUILD_CONTOURS_SIMPLIFY);
				...
				if (simplified.size()/4 >= 3)
				{
					if (cset.nconts >= maxContours)
					{
						...
						if ( ctx ) ctx->log(RC_LOG_WARNING, "rcBuildContours: Expanding max contours from %d to %d.", oldMax, maxContours);
					}
					...
					if (!cont->verts)
					{
						if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'verts' (%d).", cont->nverts);
						return false;
					}
					...
					if (!cont->rverts)
					{
						if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'rverts' (%d).", cont->nrverts);
						return false;
					}
	...
	if (cset.nconts > 0)
	{
		...
		if (!winding)
		{
			if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'hole' (%d).", cset.nconts);
			return false;
		}
		int nholes = 0;
		...
		if (nholes > 0)
		{
			...
			if (!regions)
			{
				if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'regions' (%d).", nregions);
				return false;
			}
			...
			if (!holes)
			{
				if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Out of memory 'holes' (%d).", cset.nconts);
				return false;
			}
			...
			for (int i = 0; i < cset.nconts; ++i)
			{
				...
				if (winding[i] > 0)
				{
					if (regions[cont.reg].outline)
						if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Multiple outlines for region %d.", cont.reg);
			...
			for (int i = 0; i < nregions; i++)
			{
				...
				else
				{
					...
					if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildContours: Bad outline for region %d, contour simplification is likely too aggressive.", i);
				}
			}
		}
