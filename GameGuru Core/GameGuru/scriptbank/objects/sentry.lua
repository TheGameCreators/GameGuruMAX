...
function sentry_main(e)
	...
	if g_sentry[e]['allegiance'] == 0 then	
		...
		if GetPlayerDistance(e) <= g_sentry[e]['scan_range'] and g_Entity[e]['health'] > 1 then
			...
			if sentry_EntityLookingAtPlayer(e,g_sentry[e]['scan_range'],g_sentry[e]['scan_radius']/8) == 1 then		 
				...
				if g_sentry[e]['alarm'] == 2 and g_sentry[e]['allegiance'] == 0 then 
					RotateToPlayer(e)
					current_angle[e] = g_Entity[e]['angley']
					status[e] = 'alarm'	
					if g_sentry[e]['muzzle_flash'] > 0 then
			...
			if sentry_EntityLookingAtPlayer(e,g_sentry[e]['scan_range'],g_sentry[e]['scan_radius']/8) == 0 and status[e] == 'alarm' then			
				status[e] = 'scanning'
				target_angle[e] = -1
				StopSound(e,1)			
			end
...
function sentry_EntityLookingAtPlayer(e,dis,v)
	if g_Entity[e] ~= nil then
	
		-- only when can see player
		...
		if g_Entity[e]['plrvisible'] == 1 then
			...
			return 0 -- LB: plr not visible to entity
		end
