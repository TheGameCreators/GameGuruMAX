...
public:
	...
	///  @param[in]		label	The category of the timer.
	inline rcScopedTimer(rcContext* ctx, const rcTimerLabel label) : m_ctx(ctx), m_label(label) { if ( m_ctx ) m_ctx->startTimer(m_label); }
	inline ~rcScopedTimer() { if ( m_ctx ) m_ctx->stopTimer(m_label); }
