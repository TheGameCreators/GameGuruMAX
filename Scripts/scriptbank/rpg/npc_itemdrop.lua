...
function npc_itemdrop_main(e)
	...
	if status[e] == "init" then
		...
		npc_itemdrop[e].item_no = 0
		wait[e] = g_Time + 10
		if npc_itemdrop[e].npc_name ~= "" and npc_itemdrop[e].npc_no == 0 then
		...
		if npc_itemdrop[e].item_name ~= "" and  npc_itemdrop[e].item_no == 0 then
	...
	if status[e] == "start" then
		if g_Time > wait[e] then
			ResetPosition(pEnta[e],g_Entity[pEntn[e]].x, g_Entity[pEntn[e]].y+20, g_Entity[pEntn[e]].z)
			wait[e] = g_Time + 10
		end
