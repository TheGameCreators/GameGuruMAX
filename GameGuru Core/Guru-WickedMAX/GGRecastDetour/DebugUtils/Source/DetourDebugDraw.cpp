						 ...
						 const dtMeshTile* tile, unsigned char flags)
{
	...
	//dd->begin(DU_DRAW_TRIS); // begin and end in the parent function when using tiles, otherwise only one tile gets drawn
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		const dtPoly* p = &tile->polys[i];
		...
		//LB: faster and more accurate if we just skip rendering disabled (blocked doors) polys (older method rendered them wrong) 
		if ( (p->flags & 0x10) == 0x10 ) //SAMPLE_POLYFLAGS_DISABLED) == 1)
			continue;
	...
	//dd->end(); // begin and end in the parent function when using tiles, otherwise only one tile gets drawn
	...
	// can't draw lines at the same time as vertices when using tiles
	/*
	...
	*/
	...
	/*
	// DebugDrawMAX doesn't draw points anyway
...
bool g_bRaisePolysInsteadOfDepthReadOff = false;
									  ...
									  const unsigned short polyFlags, const unsigned int col)
{
	...
	g_bRaisePolysInsteadOfDepthReadOff = true;
	for (int i = 0; i < mesh.getMaxTiles(); ++i)
	{
		const dtMeshTile* tile = mesh.getTile(i);
		if (!tile->header) continue;
	...
	g_bRaisePolysInsteadOfDepthReadOff = false;
}
...
void duDebugDrawNavMeshPoly(duDebugDraw* dd, const dtNavMesh& mesh, dtPolyRef ref, const unsigned int col)
{
	...
	else
	{
		...
		dd->begin(DU_DRAW_TRIS);
		if (g_bRaisePolysInsteadOfDepthReadOff == true)
		{	
			// this debug rendering cannot switch off depth read so cannot see blocked polys, so raise them up so we can see 'em!
			for (int i = 0; i < pd->triCount; ++i)
			{
				...
				for (int j = 0; j < 3; ++j)
				{
					if (t[j] < poly->vertCount)
						dd->vertex_raised(&tile->verts[poly->verts[t[j]] * 3], c, 1.0f);
					else
						dd->vertex_raised(&tile->detailVerts[(pd->vertBase + t[j] - poly->vertCount) * 3], c, 1.0f);
				}
			}
		}
		else
