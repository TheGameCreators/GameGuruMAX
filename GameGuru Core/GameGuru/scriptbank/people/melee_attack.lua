-- DESCRIPTION: When the target is within [RANGE=500] distance, the character will attack with <Melee Animations> the target using <Melee Weapon>, either using fists, or with a melee weapon if equipped. They will use <Sound0> and <Sound1> when attacking, and <Sound2> and <Sound3> when hurt. They can optionally [RunAway!=0] and be [InstantlyKillable!=1]. Optionally [!StarterAnimation=0] using [@StartAnimation=1(0=AnimSetList)].
...
function melee_attack_init(e)
 ...
 melee_attack_properties(e,500,0,1)
end

function melee_attack_properties(e, range, runaway, instantlykillable, starteranimation, startanimation)
...
function melee_attack_main(e)
 if g_melee_attack[e] ~= nil and g_melee_attack_behavior_count > 0 then
  g_melee_attack_behavior_count = master_interpreter_core.masterinterpreter (g_melee_attack_behavior, g_melee_attack_behavior_count, e, g_melee_attack[e], g_Entity[e])
 end
end
