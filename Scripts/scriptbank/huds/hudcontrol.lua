...
-- DESCRIPTION: [SpawnActivation!=0] if checked activates hud mode immediatley at external activation.
...
function hudcontrol_properties(e, hud_mode, mode_text, zoneheight, SpawnAtStart, SpawnActivation)
	...
	hudcontrol[e].SpawnActivation = SpawnActivation
end 
...
function hudcontrol_init(e)
	...
	hudcontrol[e].SpawnActivation = 0	
	status[e] = "init"
end
...
function hudcontrol_main(e)
	...
	if g_Entity[e]['activated'] == 1 then
		...
		if hudcontrol[e].SpawnActivation == 1 and hudcontrol[e].SpawnAtStart == 0 then
			if hudcontrol[e].hud_mode == 1 then
				PromptDuration(hudcontrol[e].mode_text,1000)
				ShowHuds()
				Destroy(e)
