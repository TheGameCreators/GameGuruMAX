...
float V_GSF_Modified(float roughness, float NdotV, float NdotL)
{
	float k = roughness / 2.0;
...
struct Surface
{
	...
	inline void update()
	{
		//roughness = clamp(roughness, 0.002, 1);
		//roughnessBRDF = roughness;

		roughness = clamp(roughness, 0.045, 1);
		roughnessBRDF = roughness * roughness;

		//roughness = clamp(roughness, 0.016, 1);
		//roughnessBRDF = pow(roughness,1.5);
		
		NdotV = saturate(abs(dot(N, V)) + 1e-5);
		
		//float f90 = saturate(50.0 * dot(f0, 0.33));
		//float f90 = 0.5 + 2.0 * NdotV*NdotV * roughnessBRDF;
		//fView = F_Schlick_Pow(NdotV);
		//F = lerp( 1, f90, fView );
		//F = f0 * F * F;
...
struct SurfaceToLight
{
	...
	inline void create(in Surface surface, in float3 Lnormalized)
	{
		...
		float f90 = 1;//max(1 - surface.roughnessBRDF, surface.f0.r);
		//float fLight = F_Schlick_Pow( NdotL );
		//Fresnel = surface.f0 * lerp( 1, f90, fLight ) * lerp( 1, f90, surface.fView );
...
float3 BRDF_GetSpecular(in Surface surface, in SurfaceToLight surfaceToLight)
{
	...
	//float D = D_GGX_Cloth(surface.roughnessBRDF, surfaceToLight.NdotH);
	...
	//float G = V_GSF_Modified(surface.roughnessBRDF, surface.NdotV, surfaceToLight.NdotL);
	//float Vis = V_3DSMax(surface.roughnessBRDF, surfaceToLight.LdotH, surfaceToLight.NdotL);

	//return min( 1, D * G * surface.f0 );
