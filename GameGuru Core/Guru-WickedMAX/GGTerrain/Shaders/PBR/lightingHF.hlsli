...
inline float3 shadowCascade(in ShaderEntity light, in float3 shadowPos, in float2 shadowUV, in uint cascade, in bool simple)
{
...
inline float3 shadowCascade(in ShaderEntity light, in float3 shadowPos, in float2 shadowUV, in uint cascade, in bool simple, in float dist)
{
	const float slice = light.GetTextureIndex() + cascade;
	const float realDistance = shadowPos.z; // bias was already applied when shadow map was rendered
	float scaleFactor = 65536.0 / (cascade + 1);
        ...
        if (light.IsCastingShadow() && surface.IsReceiveShadow() && g_xFrame_ShadowRes2D > 0)
		{
...
#endif // RTAPI
			{
				...
				else
				{
					...
					[loop]
					for (uint cascade = (simple ? 2 : 0); cascade < g_xFrame_ShadowCascadeCount; ++cascade)
					{
						...
						{
#ifdef DISTANCE_BASED_SHADOWS
							const float3 shadow_main = shadowCascade(light, ShPos, ShTex.xy, cascade, simple, dist);
#else
							const float3 shadow_main = shadowCascade(light, ShPos, ShTex.xy, cascade, simple);
#endif
...
#ifdef DISTANCE_BASED_SHADOWS
								const float3 shadow_fallback = shadowCascade(light, ShPos, ShTex.xy, cascade, simple, dist);
#else
								const float3 shadow_fallback = shadowCascade(light, ShPos, ShTex.xy, cascade, simple);
#endif
