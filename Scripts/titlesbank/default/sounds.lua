...
function sounds_init()
 ...
 if GetStoryboardActive("sounds.lua") == 0 then -- old way, if not using storyboard
 ...
 SetSpritePosition ( g_sprSliderM, 50, g_posButton[SOUNDS_MUSIC] )
   ...
   SetScreenWidgetValue("sounds.lua",SOUNDS_SOUND,g_iSoundChoice)
   SetScreenWidgetValue("sounds.lua",SOUNDS_MUSIC,g_iMusicChoice)
 end
...
function sounds_main()
 ...
 SetSpriteSize ( g_sprSliderM, (g_iMusicChoice/100)*iPercentageWidth, GetImageHeight(g_imgButtonH[SOUNDS_MUSIC]) )
    ...
    iHighlightButton = DisplayScreen("sounds.lua")  -- display storyboard screen.
	if iHighlightButton==SOUNDS_SOUND then
     g_iSoundChoice = GetScreenWidgetValue("sounds.lua",SOUNDS_SOUND)
     if g_iSoundChoice < 0 then 
      g_iSoundChoice = 0
     end
     if g_iSoundChoice > 100 then 
	...
	if iHighlightButton==SOUNDS_MUSIC then
     g_iMusicChoice = GetScreenWidgetValue("sounds.lua",SOUNDS_MUSIC)
     if g_iMusicChoice < 0 then 
      g_iMusicChoice = 0
     end
     if g_iMusicChoice > 100 then 
