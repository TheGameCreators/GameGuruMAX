...
function npc_control_properties(e, sense_text, sense_range, npc_will_flee, idle_time, attack_range, attack_interval, attack_damage, npc_can_roam, roam_range, npc_anim_speed, npc_move_speed, npc_run_speed, npc_turn_speed, npc_can_shoot, idle1_animation,  idle2_animation, idle3_animation, idle4_animation, walk_animation, run_animation, threat_animation, attack1_animation, attack2_animation, attack3_animation, shoot_animation, hurt_animation, death1_animation, death2_animation, lastflag_animation, lastflag_time, lastflag_loop, force_move, diagnostics)
	...
	npc_control[e].diagnostics = diagnostics
end
...
function npc_control_main(e)
	...
	if status[e] == "init" then
		...
		if g_Entity[e]['health'] < 100 then g_Entity[e]['health'] = g_Entity[e]['health'] + 150 end
	...
	if GetPlayerDistance(e) <= npc_control[e].sense_range and aggro[e] == 0 and allegiance[e] == 0 then
		state[e] = "sensed"
	end
	...
	if state[e] == "sensed" then
		if GetPlayerDistance(e) < npc_control[e].sense_range then
			issensed[e] = 1
			...
			if senseonce[e] == 0 then
				if npc_control[e].npc_can_roam == 1 then
					...
					PlayAnimation(e)
					PlaySound(e,2)
					PromptDuration(npc_control[e].sense_text,2000)
				end
				if npc_control[e].npc_can_roam == 2 then --No Roaming
				...
				senseonce[e] = 1
				action_delay[e] = g_Time + 1000
			end
			if npc_control[e].npc_can_roam == 1 and npc_control[e].npc_will_flee == 2 then
				if g_Time > action_delay[e] then
			...
			if npc_control[e].npc_can_roam == 1 and npc_control[e].npc_will_flee == 1 then
				aggro[e] = 0
				scare[e] = 1
				state[e] = "roam"
			end
	...
	if state[e] == "attack" then
		...
		if GetPlayerDistance(e) < npc_control[e].attack_range and g_Entity[e]['health'] > 0 and state[e] ~= "die" then
			...
			if GetTimer(e) > attack_delay[e] then
				...
				anim_var[e] = math.random(1,4)  -- Attack random animation variation
	...
	if g_Entity[e]['health'] < 100 and state[e] ~= "die" then
		animonce[e] = 0
		PlaySound(e,3)
		state[e] = "die"
	end
	...
	if npc_control[e].diagnostics == 1 then
		...
		Text(2,74,3,"Current Health : " ..g_Entity[e]['health'])
