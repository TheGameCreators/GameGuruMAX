...
#ifdef WICKEDENGINE
//PE: No need to offset vertex by half a pixel.
DARKSDK void SetVertexPosDataForSprite(int iX, int iY)
{
	...
	if (m_ptr->iXSize == 0)
	{
		m_ptr->lpVertices[0].x = (float)(fX - iXOffset);
		m_ptr->lpVertices[1].x = (float)(fX - iXOffset) + iWidth;
		m_ptr->lpVertices[2].x = (float)(fX - iXOffset);
		m_ptr->lpVertices[3].x = (float)(fX - iXOffset) + iWidth;
		m_ptr->lpVertices[0].y = (float)(iY - iYOffset);
		m_ptr->lpVertices[1].y = (float)(iY - iYOffset);
		m_ptr->lpVertices[2].y = (float)(iY - iYOffset) + iHeight;
		m_ptr->lpVertices[3].y = (float)(iY - iYOffset) + iHeight;
	}
	else
	{
		float fXSize = m_ptr->iXSize * g_fGlobalSpriteStretchX;
		float fXOffset = m_ptr->iXOffset * g_fGlobalSpriteStretchX;
		m_ptr->lpVertices[0].x = (float)(fX - fXOffset);
		m_ptr->lpVertices[1].x = (float)(fX + fXSize - m_ptr->iXOffset);
		m_ptr->lpVertices[2].x = (float)(fX - fXOffset);
		m_ptr->lpVertices[3].x = (float)(fX + fXSize - m_ptr->iXOffset);
		m_ptr->lpVertices[0].y = (float)(iY - m_ptr->iYOffset);
		m_ptr->lpVertices[1].y = (float)(iY - m_ptr->iYOffset);
		m_ptr->lpVertices[2].y = (float)(iY + m_ptr->iYSize - m_ptr->iYOffset);
		m_ptr->lpVertices[3].y = (float)(iY + m_ptr->iYSize - m_ptr->iYOffset);
	}
}
#else
DARKSDK void SetVertexPosDataForSprite ( int iX, int iY )
...
DARKSDK void RotateSpriteAtPtr( void )
{
	...
	#ifdef WICKEDENGINE
	// rotating sprites seems to mess up in MAX, new code
	SetVertexPosDataForSprite(0, 0);
	for (int iTemp = 0; iTemp < 4; iTemp++)
	{
		float fX = m_ptr->lpVertices[iTemp].x;
		float fY = m_ptr->lpVertices[iTemp].y;
		float fNewX = (float)(fX * cos (GGToRadian(fRotate)) - fY * sin (GGToRadian(fRotate)));
		float fNewY = (float)(fX * sin (GGToRadian(fRotate)) + fY * cos (GGToRadian(fRotate)));
