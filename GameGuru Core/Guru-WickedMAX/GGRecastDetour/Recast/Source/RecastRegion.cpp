								  ...
								  unsigned short* srcReg, rcIntArray& overlaps)
{
	...
	if (!regions.reserve(nreg)) {
		if ( ctx ) ctx->log(RC_LOG_ERROR, "mergeAndFilterRegions: Out of memory 'regions' (%d).", nreg);
		return false;
	}
...
{	
	...
	if (!src)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildDistanceField: Out of memory 'src' (%d).", chf.spanCount);
		return false;
	}
	unsigned short* dst = (unsigned short*)rcAlloc(sizeof(unsigned short)*chf.spanCount, RC_ALLOC_TEMP);
	if (!dst)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildDistanceField: Out of memory 'dst' (%d).", chf.spanCount);
		rcFree(src);
		return false;
	}
							...
							const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	if (!srcReg)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildRegionsMonotone: Out of memory 'src' (%d).", chf.spanCount);
		return false;
	}
	...
	if (!sweeps)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildRegionsMonotone: Out of memory 'sweeps' (%d).", nsweeps);
		return false;
	}
...
{	
	...
	if (!buf)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildRegions: Out of memory 'tmp' (%d).", chf.spanCount*4);
		return false;
	}
	
	if ( ctx ) ctx->startTimer(RC_TIMER_BUILD_REGIONS_WATERSHED);
...
//		ctx->stopTimer(RC_TIMER_DIVIDE_TO_LEVELS);
			...
			for (int j = 0; j<lvlStacks[sId].size(); j++)
			{
				...
				if (i >= 0 && srcReg[i] == 0)
				{
					if (floodRegion(x, y, i, level, regionId, chf, srcReg, srcDist, stack))
					{
						if (regionId == 0xFFFF)
						{
							if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildRegions: Region ID overflow");
							return false;
						}
	...
	if ( ctx ) ctx->stopTimer(RC_TIMER_BUILD_REGIONS_WATERSHED);
	
	{
		...
		if (overlaps.size() > 0)
		{
			if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildRegions: %d overlapping regions.", overlaps.size());
		}
	}
						 ...
						 const int borderSize, const int minRegionArea)
{
	...
	if (!srcReg)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildLayerRegions: Out of memory 'src' (%d).", chf.spanCount);
		return false;
	}
	memset(srcReg,0,sizeof(unsigned short)*chf.spanCount);
	...
	if (!sweeps)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildLayerRegions: Out of memory 'sweeps' (%d).", nsweeps);
		return false;
	}
