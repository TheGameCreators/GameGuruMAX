...
static int addEdge(rcContext* ctx, int* edges, int& nedges, const int maxEdges, int s, int t, int l, int r)
{
	if (nedges >= maxEdges)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "addEdge: Too many edges (%d/%d).", nedges, maxEdges);
		return EV_UNDEF;
	}
						 ...
						 rcIntArray& tris, rcIntArray& edges)
{
	...
	for (int i = 0; i < tris.size()/4; ++i)
	{
		...
		{
			if ( ctx ) ctx->log(RC_LOG_WARNING, "delaunayHull: Removing dangling face %d [%d,%d,%d].", i, t[0],t[1],t[2]);
							...
							rcIntArray& tris, rcIntArray& edges, rcIntArray& samples)
{
	...
	if (tris.size() == 0)
	{
		...
		if ( ctx ) ctx->log(RC_LOG_WARNING, "buildPolyDetail: Could not triangulate polygon (%d verts).", nverts);
		return true;
	}
	...
	if (ntris > MAX_TRIS)
	{
		...
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Shrinking triangle count from %d to max %d.", ntris, MAX_TRIS);
	}
									...
									rcHeightPatch& hp, rcIntArray& array)
{
	...
	while (true)
	{
		if (array.size() < 3)
		{
			if ( ctx ) ctx->log(RC_LOG_WARNING, "Walk towards polygon center failed to reach center");
			break;
		}
						   ...
						   rcPolyMeshDetail& dmesh)
{
	...
	if (!bounds)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'bounds' (%d).", mesh.npolys*4);
		return false;
	}
	rcScopedDelete<float> poly((float*)rcAlloc(sizeof(float)*nvp*3, RC_ALLOC_TEMP));
	if (!poly)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'poly' (%d).", nvp*3);
		return false;
	}
	...
	if (!hp.data)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'hp.data' (%d).", maxhw*maxhh);
		return false;
	}
	...
	if (!dmesh.meshes)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'dmesh.meshes' (%d).", dmesh.nmeshes*4);
		return false;
	}
	...
	if (!dmesh.verts)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'dmesh.verts' (%d).", vcap*3);
		return false;
	}
	dmesh.ntris = 0;
	...
	if (!dmesh.tris)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'dmesh.tris' (%d).", tcap*4);
		return false;
	}
	...
	for (int i = 0; i < mesh.npolys; ++i)
	{
		...
		if (dmesh.nverts+nverts > vcap)
		{
			...
			if (!newv)
			{
				if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'newv' (%d).", vcap*3);
				return false;
			}
			if (dmesh.nverts)
		...
		if (dmesh.ntris+ntris > tcap)
		{
			...
			if (!newt)
			{
				if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'newt' (%d).", tcap*4);
				return false;
			}
			if (dmesh.ntris)
...
bool rcMergePolyMeshDetails(rcContext* ctx, rcPolyMeshDetail** meshes, const int nmeshes, rcPolyMeshDetail& mesh)
{
	...
	if (!mesh.meshes)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'pmdtl.meshes' (%d).", maxMeshes*4);
		return false;
	}
	...
	if (!mesh.tris)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'dmesh.tris' (%d).", maxTris*4);
		return false;
	}
	...
	if (!mesh.verts)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'dmesh.verts' (%d).", maxVerts*3);
		return false;
	}
