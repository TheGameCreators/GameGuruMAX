							   ...
							   rcHeightfield& hf, rcCompactHeightfield& chf)
{
	...
	if (!chf.cells)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildCompactHeightfield: Out of memory 'chf.cells' (%d)", w*h);
		return false;
	}
	...
	if (!chf.spans)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildCompactHeightfield: Out of memory 'chf.spans' (%d)", spanCount);
		return false;
	}
	...
	if (!chf.areas)
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildCompactHeightfield: Out of memory 'chf.areas' (%d)", spanCount);
		return false;
	}
	...
	{
		if ( ctx ) ctx->log(RC_LOG_ERROR, "rcBuildCompactHeightfield: Heightfield has too many layers %d (max: %d)",
