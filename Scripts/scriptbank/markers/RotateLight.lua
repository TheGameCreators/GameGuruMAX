...
function RotateLight_main(e)
 if module_lightcontrol.control(e) == 1 then
  local useeulerangle = 1
  if useeulerangle == 1 then
   -- using direction euler
   if g_lightrotate[e]['anticlockwise'] == 0 then
    g_lightrotate[e]['angle']=g_lightrotate[e]['angle']+(g_lightrotate[e]['speed']/20.0)
   else
    g_lightrotate[e]['angle']=g_lightrotate[e]['angle']-(g_lightrotate[e]['speed']/20.0)
   end
   if g_lightrotate[e]['angle'] >= 360.0 then g_lightrotate[e]['angle'] = g_lightrotate[e]['angle'] - 360.0 end
   if g_lightrotate[e]['angle'] < 0.0 then g_lightrotate[e]['angle'] = g_lightrotate[e]['angle'] + 360.0 end
   local lightNum = GetEntityLightNumber( e )
   x, y, z = GetLightEuler(lightNum)
   local vectory = g_lightrotate[e]['angle']
   SetLightEuler(lightNum,x,vectory,z)
  else
