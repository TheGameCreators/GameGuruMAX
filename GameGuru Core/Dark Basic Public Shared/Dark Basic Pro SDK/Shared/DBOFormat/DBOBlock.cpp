...
DARKSDK_DLL bool WriteMATERIAL (D3DMATERIAL9PRETEND mMaterial, LPSTR* ppBlock, DWORD* pdwSize )
{
...
DARKSDK_DLL bool ReadMaterial (D3DMATERIAL9PRETEND* pMaterial, LPSTR* ppBlock )
{
	DWORD dwLength = sizeof(D3DMATERIAL9PRETEND);
...
DARKSDK_DLL bool ScanTextures ( sTexture* pTextureArray, DWORD dwTextureCount, LPSTR* ppBlock, DWORD* pdwSize )
{
	...
	if ( pTextureArray )
	{
		...
		for ( DWORD ti=0; ti<dwTextureCount; ti++ )
		{
			...
			#ifdef WICKEDENGINE
			WriteCODE ( DBOBLOCK_TEXTURE_CHMASK, 4, ppBlock, pdwSize );
			WriteBYTE ( pTexture->channelMask, ppBlock, pdwSize );
			WriteCR ( ppBlock, pdwSize );
			#endif
...
DARKSDK_DLL bool ScanAnimationSet ( sAnimationSet* pAnimSet, LPSTR* ppBlock, DWORD* pdwSize )
{
	...
	if ( pAnimSet )
	{
		...
		#ifdef WICKEDENGINE
		// Write additional MAX animation data into DBO
		WriteCODE(DBOBLOCK_ANIMSET_STEP1, 4, ppBlock, pdwSize);
		WriteDWORD(*(DWORD*)&pAnimSet->fAnimSetStep1, ppBlock, pdwSize);
		WriteCR(ppBlock, pdwSize);
		WriteCODE(DBOBLOCK_ANIMSET_STEP2, 4, ppBlock, pdwSize);
		WriteDWORD(*(DWORD*)&pAnimSet->fAnimSetStep2, ppBlock, pdwSize);
		WriteCR(ppBlock, pdwSize);
		WriteCODE(DBOBLOCK_ANIMSET_STEP3, 4, ppBlock, pdwSize);
		WriteDWORD(*(DWORD*)&pAnimSet->fAnimSetStep3, ppBlock, pdwSize);
		WriteCR(ppBlock, pdwSize);
		#endif
	...
	for ( DWORD ti=0; ti<dwTextureCount; ti++ )
	{
		...
		while ( dwCode > 0 )
		{
			switch ( dwCode )
			{
				...
				#ifdef WICKEDENGINE
				case DBOBLOCK_TEXTURE_CHMASK :	ReadBYTE  ( &(pTexture)->channelMask,				ppBlock ); break;
				#endif
...
DARKSDK_DLL bool ConstructAnimationSet ( sAnimationSet** ppAnimSet, LPSTR* ppBlock )
{
	...
	while ( dwCode > 0 )
	{
		switch ( dwCode )
		{
			...
			#ifdef WICKEDENGINE
			case DBOBLOCK_ANIMSET_STEP1:	ReadDWORD((DWORD*)&(*ppAnimSet)->fAnimSetStep1, ppBlock);	break;
			case DBOBLOCK_ANIMSET_STEP2:	ReadDWORD((DWORD*)&(*ppAnimSet)->fAnimSetStep2, ppBlock);	break;
			case DBOBLOCK_ANIMSET_STEP3:	ReadDWORD((DWORD*)&(*ppAnimSet)->fAnimSetStep3, ppBlock);	break;
			#endif
