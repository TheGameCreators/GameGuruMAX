...
#include "../../../Dark Basic Pro SDK/Shared/Bullet/GGTerrainPhysicsShape.h"
	...
	else
	{
		if (collisionShape->isConcave())
		{
				...
				)
			{
				...
				&& collisionShape->getShapeType() == GGTERRAIN_SHAPE_PROXYTYPE 
				)
			{
				///optimized version for GGTerrainPhysicsShape
				GGTerrainPhysicsShape* terrainShape = (GGTerrainPhysicsShape*)collisionShape;
				btTransform worldTocollisionObject = colObjWorldTransform.inverse();
				btVector3 rayFromLocal = worldTocollisionObject * rayFromTrans.getOrigin();
				btVector3 rayToLocal = worldTocollisionObject * rayToTrans.getOrigin();

				BridgeTriangleRaycastCallback rcb(rayFromLocal, rayToLocal, &resultCallback, collisionObjectWrap->getCollisionObject(), terrainShape, colObjWorldTransform);
				rcb.m_hitFraction = resultCallback.m_closestHitFraction;
				terrainShape->performRaycast(&rcb, rayFromLocal, rayToLocal);
			}
			else
			{
				//generic (slower) case
		...
		switch (shape->getShapeType())
		{
			...
			default:
			{
				...
				if (shape->isConcave())
				{
					...
					btScalar maxSize(BT_LARGE_FLOAT);
					if ( concaveMesh->getShapeType() == GGTERRAIN_SHAPE_PROXYTYPE ) maxSize = 100;
					btVector3 aabbMax(maxSize, BT_LARGE_FLOAT, maxSize);
					btVector3 aabbMin(-maxSize, -BT_LARGE_FLOAT, -maxSize);
