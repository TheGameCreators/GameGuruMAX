...
function lose_init()
 ...
 if GetStoryboardActive("lose.lua") == 0 then -- old way, if not using storyboard
...
function lose_main()
 ...
 if GetStoryboardActive("lose.lua") == 0 then -- old way, if not using storyboard
  ...
  DisplayScreen("lose.lua")  -- display storyboard screen.
  if g_sprCursorPtrClick == 1 then
   -- added to ensure can return from game over screen
   SwitchPage("")
  end
 end
end
