...
TEXTURE2DARRAY(texture_shadowarray_Spot_2d, float, TEXSLOT_SHADOWARRAY_SPOT_2D);
...
TEXTURE2DARRAY(texture_shadowarray_transparent_Spot_2d, float4, TEXSLOT_SHADOWARRAY_TRANSPARENT_SPOT_2D);
...
#define DEGAMMA(x)		pow(abs(x),2.2) // assume textures are in 2.2 gamma regardless of what gamma we use for output
...
inline float getLinearDepth(in float z, in float near, in float far)
{
	...
	// simplified
	float lin = (1 - z) * near / far; // if far goes to infinity then this goes to 0
	lin += z;
	return near / lin;
}
