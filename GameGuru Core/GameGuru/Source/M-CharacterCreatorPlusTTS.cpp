...
int ConvertWAVtoLIP ( char* pAbsWAVFile )
{
	...
	// if not in writables, must be in master folder
	if (FileExist(pRealAbsWAVFile) == 0)
	{
		// point to read only audiobank 
		strcpy(pRealAbsWAVFile, g.fpscrootdir_s.Get());
		strcat(pRealAbsWAVFile, "\\Files\\");
		strcat(pRealAbsWAVFile, pAbsWAVFile);
		if (FileExist(pRealAbsWAVFile) == 0)
		{
			// file does not exist in read only or writables!
			return -1;
		}
	}
