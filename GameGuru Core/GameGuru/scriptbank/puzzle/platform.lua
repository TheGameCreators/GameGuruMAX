...
--Platform Script V12  by Necrym59 with thanks to smallg
...
function platform_properties(e, prompt_text, direction_x, direction_y, direction_z, distance_x, distance_y, distance_z, speed_x, speed_y, speed_z, lock_x_position, lock_y_position, lock_z_position, lock_x_rotation, lock_y_rotation, lock_z_rotation, auto_eject, visibility, riding, mode)
		...
		if platform[e].direction_x == 1 then 
			if direction[e][1] > 0 then 
				if x1 < startx[e] + platform[e].distance_x then 
					x1 = x1 + direction[e][1]
					if x1 >= startx[e] + platform[e].distance_x then
						if platform[e].auto_eject == 1 then ForcePlayer(g_PlayerAngY + 1, 5) end
						StopSound(e,1)
						PlaySound(e,2)
						wait_time[e] = GetTimer(e) + 3000
						state[e] = "idle"
