...
bool InputGeom::loadData(class rcContext* ctx, float* pVertices, uint32_t numVertices)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
	...
	if (!m_mesh->loadData(pVertices, numVertices))
	{
		ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load data");
		return false;
	}
...
bool InputGeom::raycastMesh(float* src, float* dst, float& tmin)
{
	...
	int* cid = 0;
	const int ncid = rcGetChunksOverlappingSegment(m_chunkyMesh, p, q, &cid);
	if ( !cid ) return false;
	else if (!ncid)
	{
		delete [] cid;
