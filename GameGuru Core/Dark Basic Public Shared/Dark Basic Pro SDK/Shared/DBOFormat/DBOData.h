...
struct sTexture
{
	...
	#ifdef WICKEDENGINE
	// used by importer for metalness and roughness textures, low 4 bits specify which channels exist, next 2 bits specify the channel index to use, top 2 bits unused
	unsigned char					channelMask;								// [0-3]=channels, [4-5]=channel index, [6-7]=unused
	unsigned char					reservechar2;
	unsigned char					reservechar3;
	unsigned char					reservechar4;
	#else
	DWORD							dwReservedT3;
	#endif
...
// Recreate old DX9 structures so can remove legacy dependencies
struct D3DCOLORVALUEPRETEND
{
	float r;
	float g;
	float b;
...
struct D3DMATERIAL9PRETEND
{
	D3DCOLORVALUEPRETEND   Diffuse;        /* Diffuse color RGBA */
	D3DCOLORVALUEPRETEND   Ambient;        /* Ambient color RGB */
	D3DCOLORVALUEPRETEND   Specular;       /* Specular 'shininess' */
	D3DCOLORVALUEPRETEND   Emissive;       /* Emissive color RGB */
	float				   Power;          /* Sharpness if specular highlight */
};
struct D3DXMATERIALPRETEND
{
	D3DMATERIAL9PRETEND  MatD3D;
...
struct sMultiMaterial
{
	...
	D3DMATERIAL9PRETEND				mMaterial;									// unique material information
...
struct sMeshTexture
{
	...
	D3DXMATERIALPRETEND*			pMaterialBank;								// temp store for all materials of a single mesh
	D3DMATERIAL9PRETEND				mMaterial;									// unique material information
					  ...
					  sMeshMultipleAnimation
{
	...
	#ifdef WICKEDENGINE
	uint64_t						wickedmeshindex;
	uint64_t						wickedmaterialindex;
	uint64_t						wickedarmatureindex;
	sFrame*							pFrameAttachedTo;
	uint64_t						master_wickedmeshindex;
						   ...
						   sAnimationExtraBones
{
	...
	#ifdef WICKEDENGINE
	int								wickedanimationchannel[3];
	int								wickedanimationsampler[3];
	#endif
...
struct sAnimationSet
{
	...
	#ifdef WICKEDENGINE
	float							fAnimSetStep1;								// LB: store footfall left frame
	float							fAnimSetStep2;								// LB: store footfall right frame
	float							fAnimSetStep3;								// LB: store footfall any frame
	float							fAnimSetRes1;								// LB: reserved
	float							fAnimSetRes2;								// LB: reserved
	float							fAnimSetRes3;								// LB: reserved
	#endif

	#ifdef WICKEDENGINE
	uint64_t						wickedanimentityindex;
	#endif
						...
						sObjectCustom
{
	...
	// Wicked References in Object Structure
	#ifdef WICKEDENGINE
	uint64_t						wickedrootentityindex;
	void*							wickedloaderstateptr;
	bool							bUseFixedSize;
	GGVECTOR3						vecFixedSize;
	#endif
};
