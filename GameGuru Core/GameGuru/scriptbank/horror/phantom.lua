...
-- DESCRIPTION: [IDLE_ANIMATION$="Zombie_Heavy_Breathe"] [MOVE_ANIMATION$="Zombie_Run_01"]
...
function phantom_properties(e, prompt_text, appearance_mode, appearance_level, appearance_aura, appearance_speed, appearance_time, disappearance_mode, movement_speed, movement_x, movement_z, idle_animation, move_animation)
	...
	phantom[e].idle_animation = idle_animation
	phantom[e].move_animation = move_animation
end 
...
function phantom_init(e)
	...
	phantom[e].idle_animation = "Zombie_Heavy_Breathe"
	phantom[e].move_animation = "Zombie_Run_01"
	status[e] = "init"
	doonce = 0
	rotonce = 0
	animonce = 0
...
function phantom_main(e)
	...
	if g_Entity[e]['activated'] == 1 then
		...
		if phantom[e].appearance_mode == 2 then -- Show and Approach
			...
			if current_level[e] >= phantom[e].appearance_level then	
				if animonce == 0 then				
					...
					LoopAnimationFrom(e,math.random()*100)					
					animonce = 1
				end
				...
				SetAnimationSpeedModulation (e,phantom[e].movement_speed/100)
				StopSound(e,0)				
		...
		if phantom[e].appearance_mode == 3 then -- Show-Face and Approach	
			...
			if current_level[e] >= phantom[e].appearance_level and rotonce == 0 then
				CollisionOff(e)				
				RotateToPlayer(e)			
				CollisionOn(e)
				rotonce = 1
			end			
			if current_level[e] >= phantom[e].appearance_level and rotonce == 1 then
				if animonce == 0 then				
					SetAnimationName(e,phantom[e].move_animation)								
					LoopAnimationFrom(e,math.random()*100)					
					animonce = 1
