...
bool BaseItemManager::RemoveItem(int id)
{
	...
	if(index > -1 && index < m_data.size())
	{
		...
		//m_data.remove2(item);
		m_data.remove3(item); //LB: we upgraded to BULLET3, so to play it safe we renamed to remove3
		SAFE_DELETE(item);
		return true;
	}
	return false;
...
int BaseItemManager::Find(int id)
{
	...
	int index = m_data.findLinearSearch3(temp); //LB: when upgraded to BULLET3, this function code was altered, so put old stuff back
