...
void sound_footfallsound_core (int iPlayerOrCharacter, int footfalltype, float fX, float fY, float fZ, int iLeftOrRight, int iWalkOrRun, int* lastfootfallsound)
{
	int matSoundIndex = 0;
	if (iWalkOrRun == 0)
	{
		if (iLeftOrRight == 0)
			matSoundIndex = matSound_WalkLeft;
		else
			matSoundIndex = matSound_WalkRight;
	}
	else
	{
		if (iLeftOrRight == 1)
			matSoundIndex = matSound_RunLeft;
		else
			matSoundIndex = matSound_RunRight;
	}
	if (iLeftOrRight == 1)
	{
		// only advance on right foot down
		(*lastfootfallsound) = (*lastfootfallsound) + 1;
