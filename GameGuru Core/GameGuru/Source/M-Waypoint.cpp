		...
		if (bOnlyFollowTerrain)
		{
			if (t.terrain.TerrainID > 0 || ObjectExist(t.terrain.terrainobjectindex))
			{
				...
				if (ObjectExist(t.terrain.terrainobjectindex))
					...
					tobj = t.terrain.TerrainID;

				sObject* pObject = GetObjectData(tobj);
				if (pObject)
				{
					WickedCall_SetObjectRenderLayer(pObject, GGRENDERLAYERS_TERRAIN);
					float fPickedYAxis = 0.0f;
					WickedCall_GetPick(&t.tx_f, &fPickedYAxis, &t.tz_f, NULL, NULL, NULL, NULL, GGRENDERLAYERS_TERRAIN);
			...
			else
			{
				...
				if (t.onedragmode == 1 && (t.mx_f != t.onedragx_f || t.mz_f != t.onedragz_f))
				{
					...
					if (t.linkto > 0)
					{
						...
						t.waypointcoord[t.linkto].y = fWaypointHeightNow;
...
void waypoint_movetothiscoordinate ( void )
{
	...
	for ( t.ttw = t.waypoint[t.waypointindex].start ; t.ttw <= t.waypoint[t.waypointindex].finish; t.ttw++ )
	{
		...
		{
			// special rule which finds the terrain heights at the waypoint nodes if center coordinate is with X of terrain floor
			// (to preserve zones which are above the terrain for things like building second floors, etc)
			float fGroundHeight = BT_GetGroundHeight(0, t.thisx_f, t.thisz_f);
			float fDistanceFromTerrain = t.thisy_f - fGroundHeight;
			if (fabs(fDistanceFromTerrain) < 10.0f)
			{
				// have waypoint nodes follow the terrain
	...
	if (  obj <= g.editorwaypointoffsetmax ) 
	{
		...
		if (  t.waypoint[waypointindex].count>0 ) 
		{
			...
			if (polycount == 0)
			{
				...
				WickedCall_PresetObjectRenderLayer(GGRENDERLAYERS_CURSOROBJECT);
				MakeObjectSphere(obj, 10.0f);
				WickedCall_PresetObjectRenderLayer(GGRENDERLAYERS_NORMAL);
				int w = t.waypoint[waypointindex].start;
				wx_f = t.waypointcoord[w].x;
				wy_f = t.waypointcoord[w].y + fHeightFromFloor;
				wz_f = t.waypointcoord[w].z;
				diffuse = Rgb(255, 255, 255);
				PositionObject(obj, wx_f, wy_f-2, wz_f);
				ShowObject(obj);
				SetObjectCollisionOff(obj);
			}
			// wicked instead, sets color of entire waypoint object
			if ( t.waypoint[waypointindex].fillcolor == 1 ) diffuse = Rgb(255*fade_f,128*fade_f,0*fade_f);
			if ( t.waypoint[waypointindex].fillcolor == 2 ) diffuse = Rgb(183*fade_f,220*fade_f,244*fade_f);
			if ( t.waypoint[waypointindex].fillcolor == 3 ) diffuse = Rgb(0*fade_f,128*fade_f,255*fade_f);
			if ( t.waypoint[waypointindex].fillcolor == 4 ) diffuse = Rgb(0*fade_f,255*fade_f,0*fade_f);
			if ( t.waypoint[waypointindex].fillcolor == 5 ) diffuse = Rgb(255*fade_f,0*fade_f,0*fade_f);
			if ( t.waypoint[waypointindex].fillcolor == 6 ) diffuse = Rgb(64*fade_f,64*fade_f,64*fade_f);
			if ( t.waypoint[waypointindex].fillcolor == 7 ) diffuse = Rgb(0*fade_f,0*fade_f,255*fade_f);
...
void finalisepolymesh ( int obj )
{
	...
	DWORD diffuse = Rgb(255, 255, 255);
	if (MemblockExist(memblock) == 1)
	{
		int iPositionOfColor = 12 + (12+12); // XYZ + NORM + !DIFFUSE!
		//PE: Polycount 0 gives errors , handled later.
		if (GetMemblockSize(memblock) > iPositionOfColor+3)
			diffuse = ReadMemblockDWord(memblock, iPositionOfColor);
	}
	SetObjectDiffuse (obj, Rgb(0, 0, 0));
	SetObjectEmissive (obj, diffuse);
	#endif
...
{
	*waypoint = t.waypoint[index];

	for (int coordIndex = waypoint->start; coordIndex <= waypoint->finish; coordIndex++)
	{
		coords[coordIndex - waypoint->start] = t.waypointcoord[coordIndex];
	}
}

// Create a new waypoint object using data from an undo or redo DELETE event. // For zones only (style == 2).
void waypoint_undoredo_add(const waypointtype& waypoint, waypointcoordtype* coords)
{
	// Create a new waypoint strand (or find empty one)
	if (g.waypointcoordmax < 0) g.waypointcoordmax = 0;
	for (t.tw = 1; t.tw <= g.waypointmax; t.tw++)
	...
	// Set Waypoint style and data
	t.waypoint[t.waypointindex].style = waypoint.style;
	t.waypoint[t.waypointindex].fillcolor = waypoint.fillcolor;
	t.waypoint[t.waypointindex].linkedtoentityindex = waypoint.linkedtoentityindex;
	t.waypoint[t.waypointindex].count = waypoint.count;
	t.waypoint[t.waypointindex].start = g.waypointcoordmax + 1;
	int coordIndex = 0;
	for (t.ttii = 1; t.ttii <= t.waypoint[t.waypointindex].count; t.ttii++)
	{
		...
		Dim(t.waypointcoord, g.waypointcoordmax);
		t.w = g.waypointcoordmax;
		t.waypointcoord[t.w].x = coords[coordIndex].x;
		t.waypointcoord[t.w].z = coords[coordIndex].z;
		t.waypointcoord[t.w].y = coords[coordIndex].y;
		t.waypointcoord[t.w].index = t.waypointindex;
		t.waypointcoord[t.w].link = 0;
		coordIndex++;
	}
