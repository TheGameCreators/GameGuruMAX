-- Textinzone v5  
-- DESCRIPTION: [PROMPT_TEXT$=""] is displayed on screen while the player is within the zone 
...
function textinzone_properties(e, prompt_text, zoneheight, onlyonce, spawnatstart, period)
...
function textinzone_main(e)
	...
	if g_Entity[e]['activated'] == 1 then
		if g_Entity[e]['plrinzone'] == 1 and multiuse[e] == 0 and g_PlayerPosY > g_Entity[e]['y'] and g_PlayerPosY < g_Entity[e]['y']+textinzone[e].zoneheight then
			if doonce[e] == 0 then
				PromptDuration(textinzone[e].prompt_text,textinzone[e].period * 1000)
				wait[e] = g_Time + textinzone[e].period * 1000
				doonce[e] = 1
			end	
		end
